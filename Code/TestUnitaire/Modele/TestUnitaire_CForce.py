...
class TestUnitaire_CForce(unittest.TestCase) :
    ...
    def test_VitesseAlphaMax_is_OK(self):
        ...
        self.assertEqual(1.742, round(self.force.FORVitesseAlphaMax(1.34), 3))

    def test_VecteurVitesse_is_OK(self):
        t = 4
        vecteurVitesse = self.force.FORVecteurVitesse(self.Position, self.PositionDeltaT, t)
    ...
    def test_eAlpha_is_OK(self) :
        eAlpha =self.force.FOReAlpha(self.vRkAlpha, self.Position)
        self.assertEqual(-0.66896473, round(eAlpha[0],8))
        self.assertEqual(-0.74329415, round(eAlpha[1],8))

    def test_b_is_OK(self) :
        resultat = self.force.FORb(np.array([10, 15]), np.array([11, 10]), 1.34, np.array([1, 5]))
        self.assertEqual(5.57,round(resultat,2))

    def test_VAlphaBeta_is_OK(self):
        self.assertEqual(3.4, round(self.force.FORVAlphaBeta(4) * (10 ** (6)), 2))

    def test_UAlphaObstacle_is_OK(self):

        resultat = self.force.FORUAlphaObstacle(4)
