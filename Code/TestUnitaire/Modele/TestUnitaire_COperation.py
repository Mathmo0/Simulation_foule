...
class TestUnitaire_COperation(unittest.TestCase) :
    ...
    def test_DetectionCercle_is_OK(self) :
        self.assertTrue(COperation.OPEDetectionCercle(0, 0, 1, 1, 3))

    def test_DetectionCercle_is_not_OK(self):
        self.assertFalse(COperation.OPEDetectionCercle(0, 0, 3, 3, 1.5))

    def test_FonctionTrajectoirePieton_is_OK(self):

        resultat = COperation.OPEFonctionTrajectoirePieton(self.Position, self.PositionDeltaT)
        self.assertEqual(resultat[0],2)
        self.assertEqual(resultat[1],1)

    def test_Nabla_is_OK(self):

        resultat = COperation.OPENabla(self.Position, self.PositionDeltaT, self.PositionB)
